...
export interface Pack {
  ...
  isQuickPack?: boolean;
  // true면 이 팩은 가방 상단 "AI" 메뉴 > "AI 추천"(날씨/명소/맛집/특산물)에서 "+추가"로
  // 만들어진 전용 팩(이름은 보통 "AI추천")이다. AI 추천 자체가 프리미엄 전용 기능이라, 같은
  // 가방을 보는 다른 멤버가 무료회원이면 이 팩을 화면에서 숨긴다(lib/premiumLimits.ts의
  // getViewablePacks) - 프리미엄 회원의 결과물을 무료회원이 그대로 얻어가는 것을 막기 위함.
  // 반드시 이 플래그로만 판별하고 pack.name("AI추천")으로 판별하지 않는다 - 사용자가 직접
  // "AI추천"이라는 이름의 팩을 만들어도 이 플래그가 없으므로 정상적으로 계속 보인다(구분 목적).
  aiRecommendSource?: boolean;
  ...
  // 이 패(kind==='editor', 메모패)에 첨부한 사진/PDF URL 배열. 가방 안의 메모패에서만
  // 쓰는 기능(bagId가 있을 때만 UI에 노출). Bag.images와 동일하게 최대
  // MAX_PACK_IMAGES(lib/premiumLimits.ts)개까지, PDF는 프리미엄 전용.
  images?: string[];
...
export interface Bag {
  ...
  locked?: boolean;
  // [미사용 - deprecated] 예전에는 날씨/AI 추천을 이 필드로 ON/OFF 토글했는데, 지금은 날씨+AI 추천 자체가
