...
export default function MemoDocViewer({ pack, className = "" }: MemoDocViewerProps) {
  ...
  return (
    <div className={`w-full flex flex-col gap-4 text-foreground ${className}`}>
      {/* 첨부 이미지/파일 갤러리 */}
      {images.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {images.map((src, idx) => {
            ...
            return (
              <div
                key={idx}
                className="relative h-28 w-28 shrink-0 rounded-xl overflow-hidden border border-border bg-surface-2 shadow-xs"
              >
                {isImg ? (
                  <img
                    src={src}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-mono uppercase truncate max-w-full">
      ...
      {/* TipTap 서식 본문 (읽기 전용) */}
      <div className="prose prose-sm dark:prose-invert max-w-none font-sans leading-relaxed text-[13.5px]">
