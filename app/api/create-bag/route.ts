...
//
// 왜 서버가 필요한가: "무료는 동시에 3개까지" 제한을 클라이언트에서만 검사하면, devtools로
// 직접 Firestore에 write하거나 검사 코드를 건너뛰어 무제한으로 만들 수 있다. 그래서
// firestore.rules에서 bags 컬렉션의 client-side create를 막아두고(allow create: if false),
// 실제 생성은 이 라우트(Admin SDK, 클라이언트가 우회 불가)만 할 수 있게 한다.
